...
if ( vamtam_has_woocommerce() ) {
	...
	function vamtam_woocommerce_cart_item_quantity( $content, $cart_item_key, $cart_item ) {
		...
		if ( ! $only_one_allowed && is_cart() ) {
			$select_cart_attrs = 'name="cart[' . esc_attr( $cart_item_key ) . '][qty]" value="' . esc_attr( $cart_item['quantity'] ) . '" title="' . esc_attr__( 'Qty', 'lagar' ) . '" min="0" max="' . esc_attr( $_product->get_max_purchase_quantity() ) . '"';
		}
		...
		?>
			<div class="vamtam-empty-cart">
				...
				<div class="empty-cart-notice">
					<h5><?php esc_html_e( 'Your cart is currently empty.', 'lagar' ); ?></h5>
				</div>
			</div>
		<?php
	}
		...
		?>
		    <?php if ( current_filter() === 'product_cat_edit_form_fields') : ?>
				<tr class="form-field">
					<th valign="top" scope="row">
						<label for="term_fields[class_level]"><?php echo apply_filters( 'vamtam_product_cat_class_level_label', esc_html__( 'Class Level', 'lagar' ) ); ?></label>
					</th>
					<td>
						<input type="text" size="40" value="<?php echo esc_attr( $class_level ); ?>" id="term_fields[class_level]" name="term_fields[class_level]"><br/>
						<span class="description"><?php echo apply_filters( 'vamtam_product_cat_class_level_description', esc_html__( 'e.g. Beginner, Advanced etc. ', 'lagar' ) ); ?></span>
					</td>
				</tr>
				<tr class="form-field">
					<th valign="top" scope="row">
						<label for="term_fields[class_category]"><?php echo apply_filters( 'vamtam_product_cat_class_category_label', esc_html__( 'Class Category', 'lagar' ) ); ?></label>
					</th>
					<td>
						...
						<span class="description"><?php echo apply_filters( 'vamtam_product_cat_class_category_description', esc_html__( 'e.g. A1, B2 etc.', 'lagar' ) ); ?></span>
					</td>
				</tr>
				<tr class="form-field">
					<th valign="top" scope="row">
						<label for="term_fields[class_description]"><?php echo apply_filters( 'vamtam_product_cat_class_description_label', esc_html__( 'Class Description', 'lagar' ) ); ?></label>
					</th>
					<td>
						...
						<span class="description"><?php echo apply_filters( 'vamtam_product_cat_class_description_description', esc_html__( 'Add a description for this class.', 'lagar' ) ); ?></span>
					</td>
				</tr>
			<?php elseif ( current_filter() === 'product_cat_add_form_fields' ) : ?>
				...
				<div class="form-field">
					...
					<span class="description"><?php echo apply_filters( 'vamtam_product_cat_class_description_description', esc_html__( 'e.g. A1, B2 etc.', 'lagar' ) ); ?></span>
				</div>
				<div class="form-field">
					<label for="term_fields[class_description]"><?php echo apply_filters( 'vamtam_product_cat_class_description_label', esc_html__( 'Class Description', 'lagar' ) ); ?></label>
	...
	function vamtam_woocommerce_after_subcategory_title( $term ) {
		...
		<?php if ( $show_btn ) : ?>
				<div class="vamtam-hover-wrap">
					<span class="vamtam-product-cat-btn button"><?php echo apply_filters( 'vamtam_template_loop_product_cat_button_text', esc_html__( 'See more', 'lagar' ) ); ?></span>
				</div>
			<?php endif; ?>
		<?php
	}
	...
	// Product's gallery custom thumbnail size.
	if ( wc_string_to_bool( get_option( 'vamtam_wc_use_custom_product_gallery_thumbnail_image_size', apply_filters( 'vamtam_wc_use_custom_product_gallery_thumbnail_image_size', 'no' ) ) ) ) {
		function vamtam_woocommerce_image_size_gallery_thumbnail( $size ) {
			return array(
				'width'  => get_option( 'vamtam_wc_product_gallery_thumbnail_image_width', apply_filters( 'vamtam_wc_product_gallery_thumbnail_default_image_width', 200 ) ),
				'height' => '',
				'crop'   => 0,
			);
		}
		add_filter( 'woocommerce_get_image_size_gallery_thumbnail', 'vamtam_woocommerce_image_size_gallery_thumbnail', 100, 1 );
	}
...
class Vamtam_WC_Ajax_Add_To_Cart_Handler {
	...
	public static function handle_grouped_products() {
		...
		if ( ! empty( $items ) ) {
			...
			if ( ! $was_added_to_cart && ! $quantity_set ) {
				wc_add_notice( __( 'Please choose the quantity of items you wish to add to your cart.', 'lagar' ), 'error' );
	...
	public static function handle_variable_products() {
		...
		if ( empty( $variation_id ) && $product && $product->is_type( 'variable' ) ) {
			...
			wc_add_notice( sprintf( __( 'Please choose product options by visiting <a href="%1$s" title="%2$s">%2$s</a>.', 'lagar' ), esc_url( get_permalink( $product_id ) ), esc_html( $product->get_name() ) ), 'error' );
