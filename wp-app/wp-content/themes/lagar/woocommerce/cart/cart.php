...
if ( ! function_exists( 'vamtam_render_cart_item' ) ) {
	function vamtam_render_cart_item( $cart_item_key, $cart_item ) {
		...
		<div class="vamtam-cart__product woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
			...
			<div class="vamtam-cart__product-name product-name" data-title="<?php esc_attr_e( 'Product', 'lagar' ); ?>">
				<?php
				if ( ! $product_permalink ) {
				...
				if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $cart_item['quantity'] ) ) {
					echo wp_kses_post( apply_filters( 'woocommerce_cart_item_backorder_notification', '<p class="backorder_notification">' . esc_html__( 'Available on backorder', 'lagar' ) . '</p>', $product_id ) );
				}
				?>
			</div>

			<div class="vamtam-cart__product-price product-price" data-title="<?php esc_attr_e( 'Price', 'lagar' ); ?>">
				<?php
			...
			<div class="vamtam-cart__product-remove product-remove">
				...
				echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf(
					...
					esc_attr( sprintf( __( 'Remove %s from cart', 'lagar' ), wp_strip_all_tags( $product_name ) ) ),
					esc_attr( $product_id ),
	...
	<form class="woocommerce-cart-form" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
		<div class="vamtam-cart-main">
			<div class="vamtam-cart__header">
				<span class="font-h4 label"><?php esc_html_e( 'Your cart ', 'lagar' ); ?></span>
				<span class="font-h4 item-count">(<?php echo esc_html( sprintf( _n( '%s item', '%s items', WC()->cart->get_cart_contents_count(), 'lagar' ), WC()->cart->get_cart_contents_count() ) ); ?>)</span>
			</div>
			...
			<div class="cart-contents">
				...
				<input class="hidden" type="submit" class="button" name="update_cart" value="<?php esc_attr_e( 'Update cart', 'lagar' ); ?>">

				<?php if ( wc_coupons_enabled() ) { ?>
					<input class="hidden" type="submit" name="apply_coupon" value="<?php esc_attr_e( 'Redeem', 'lagar' ); ?>">
				<?php } ?>
			...
			<?php if ( wc_coupons_enabled() ) { ?>
				<div class="coupon">
					<details>
						<summary>
							<span class="label">
								<?php esc_html_e( 'Add a voucher', 'lagar' ); ?>
								<span><?php esc_html_e( '(Optional)', 'lagar' ); ?></span>
							</span>
						...
						<div class="content">
							<input type="text" name="coupon_code" class="input-text" id="coupon_code" value="" placeholder="<?php esc_attr_e( 'Enter your code', 'lagar' ); ?>" />
							<button type="submit" name="apply_coupon" value="<?php esc_attr_e( 'Redeem', 'lagar' ); ?>">
								<?php esc_html_e( 'Redeem', 'lagar' ); ?>
							</button>
						</div>
					</details>
